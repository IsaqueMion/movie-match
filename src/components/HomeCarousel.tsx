      ...
      {slides.length > 1 && (
        <>
          <button onClick={prev} aria-label="Anterior"
            className="absolute left-3 top-1/2 -translate-y-1/2 z-40 bg-white/15 hover:bg-white/25 transition px-3 py-2 rounded-full backdrop-blur text-white">‹</button>
          <button onClick={next} aria-label="Próximo"
            className="absolute right-3 top-1/2 -translate-y-1/2 z-40 bg-white/15 hover:bg-white/25 transition px-3 py-2 rounded-full backdrop-blur text-white">›</button>
        </>
      )}
